...
/**
 * K-means聚类结果可视化工具 使用AWT绘制聚类结果，包括实体位置、聚类中心和聚类区域
 */
public class KmeansVisualizer extends Frame {

    // 聚类数据
    private SEUKMeansClustering clustering;
     ...
     * 构造函数
     *
     * @param clustering K-means聚类实例
     * @param entities 所有要可视化的实体
     */
    public KmeansVisualizer(SEUKMeansClustering clustering, List<StandardEntity> entities) {
    ...
    /**
     * 绘制实体形状边框 使用Area实体的顶点列表绘制其实际几何形状
     */
    ...
    private void drawClusters(Graphics2D g2d) {
        ...
        Color[] clusterColors = generateClusterColors(clusterCount);
                ...
                if (memberCount > 0) {
                    ...
                    for (EntityID memberId : members) {
                        ...
                        if (member instanceof Area) {
                            ...
                            // 绘制成员点（用聚类颜色）
                            g2d.fillOval(memberScreen.x - POINT_SIZE / 2, memberScreen.y - POINT_SIZE / 2, POINT_SIZE, POINT_SIZE);
                        }
                    }
                }
            }
     ...
     */
    private void drawClusterBoundary(Graphics2D g2d, int clusterIndex, Color color) {
     ...
     * 生成聚类颜色
     */
    private Color[] generateClusterColors(int count) {
        Color[] colors = new Color[count];
        float hueStep = 1.0f / count;

        for (int i = 0; i < count; i++) {
            colors[i] = Color.getHSBColor(i * hueStep, 0.8f, 0.9f);
        }
        return colors;
    }
     ...
     * 显示实体信息
     */
    private void showEntityInfo(int screenX, int screenY) {
        // 查找点击的实体
        for (StandardEntity entity : allEntities) {
            if (entity instanceof Area) {
                Area area = (Area) entity;
    ...
    public void saveScreenshot() {
        try {
            ...
            String fileName = "clustering_" + this.clustering.getUrn() + "_" + timestamp + ".png";
    ...
    public void visualize() {
        ...
        // 延迟一段时间后自动保存画面
        try {
            Thread.sleep(2000); // 等待3秒确保画面完全渲染
            saveScreenshot();
